...
static int g_iFdHZK;
static unsigned char *g_pucHZKMem;   //字库起始地址
static unsigned char *g_pucHZKMemEnd;//字库结束地址

static int 
GBKFontInit(char *pcFontFile, unsigned int dwFontSize)
{
	struct stat tStat;
	...
	g_pucHZKMem = (unsigned char *)mmap(NULL , tStat.st_size, \
		PROT_READ, MAP_SHARED, g_iFdHZK, 0);
...
static int 
GBKGetFontBitmap(unsigned int dwCode, PT_FontBitMap ptFontBitMap)
{
	int iArea;
	int iWhere;
