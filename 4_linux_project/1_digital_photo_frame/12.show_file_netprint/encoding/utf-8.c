...
static int 
isUtf8Coding(unsigned char *pucBufHead)
{
 ...
 */
static int 
GetPreOneBits(unsigned char ucVal)
{
	int i;
	int j = 0;
...
//返回得到字形编码的字节数
static int 
Utf8GetCodeFrmBuf(unsigned char *pucBufStart, unsigned char *pucBufEnd, unsigned int *pdwCode)
{
#if 0
	...
	else
	{
		...
		for (i = 1; i < iNum; i++)
		{
			ucVal = pucBufStart[i] & 0x3f;//b 0011 1111
			//dwSum是16位的无符号整形数，左移6位来存入上面的ucVal。
			dwSum = dwSum << 6;
			//dwSum中存入ucVal。
			dwSum += ucVal;			
		}
		//0100 1110     0010 1101：其余剩下的用来表示unicode码0x4E2D。
		*pdwCode = dwSum;
