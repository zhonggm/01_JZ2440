...
static int usbmouse_as_key_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	/* d. 硬件相关操作 */
	/* 数据传输3要素: 源,目的,长度 */
	/* 源: USB设备的某个端点，这个源既含中断标志位，又含设备号端点号，以及方向 */
	...
	uk_urb->transfer_dma = usb_buf_phys;//USB主机得到数据后会往某个内存里写，写到哪，就要告诉它要写的物理地址了。
	uk_urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;//设置标志
