...
{
	/* 1. 分配一个空间给net_device结构体 */
	vnet_dev = alloc_netdev(0, "vnet%d", ether_setup);  /* alloc_etherdev */

	/* 2. 设置 */
//	vnet_dev->hard_start_xmit = virt_net_send_packet; //kernel 2.6.22.6
	vnet_dev->netdev_ops = &vnet_dev_ops;//kernel 3.4.2
...
{
	unregister_netdev(vnet_dev);//注销
	free_netdev(vnet_dev);      //释放
}
