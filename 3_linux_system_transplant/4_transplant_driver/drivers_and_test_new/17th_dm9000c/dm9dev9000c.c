...
int __init dmfe_probe1(struct net_device *dev)
{
	...
	do {
		/* 2440怎么分辨操作是索引寄存器还是数据寄存器呢? 设置cmd引脚，
		想读某个寄存器时先把这个地址写个索引寄存器里面，再来读所谓的数据寄存器 */
		...
		if (id_val == DM9KS_ID || id_val == DM9010_ID) {
			...
			//jz2440用的网卡确实不符合这个条件，但也可以用， zhong.gm@qq.com
...
static int dmfe_open(struct net_device *dev)
{
	...
	//设置触发条件，把第3个实参0改为IRQF_TRIGGER_RISING，zhong.gm@qq.com
...
int __init dm9000c_init(void)
{
	...
	iobase = (int)ioremap(0x20000000, 1024);/* zhong.gm@qq.com */
	irq = IRQ_EINT7;/* zhong.gm@qq.com */

	/* 设置S3C2440的memory controller */
	bwscon	 = ioremap(0x48000000, 4);
	bankcon4 = ioremap(0x48000014, 4);

	/* DW4[17:16]: 01-16bit位宽
	...
	*bwscon = val;
	 ...
	 *			   DM9000C进行读操作时, nOE变为高电平之后, 数据线上的数据最多要维持6ns
