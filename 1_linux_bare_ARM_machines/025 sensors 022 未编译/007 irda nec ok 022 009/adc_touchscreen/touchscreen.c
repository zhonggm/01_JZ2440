...
/* 读原始数据 */
void ts_read_raw(int *px, int *py, int *pressure)
{
...
void touchsrceen_timer_irq(void)
{
	...
	if (is_in_auto_mode())/* 如果定时器中断在ADC中间产生，应该返回 */
			return;
...
void Isr_Adc(void)
{
	...
	/* 只有在"等待中断模式"下才可以使用ADCDAT0'BIT 15来判断触摸笔状态，所以要先设置为"等待中断模式" */
	...
	//enter_wait_pen_up_mode();/* 不够16次启动ADC时不应该进入这个模式而应该保持自动测量模式, 它会影响数据 */
}
