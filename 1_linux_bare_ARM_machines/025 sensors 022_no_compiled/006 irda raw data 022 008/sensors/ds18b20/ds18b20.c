...
static int ds18b20_initialization(void)
{
	...
	udelay(80);//DS18B20 WAITS 15-60us

	val = ds18b20_data_get();
	udelay(250);//wait 60+ pulse 240- waited 80 = 240us，取250us
	return val;
}
...
static void ds18b20_write_bit(int val)
{
	if (0 == val)
	{
		...
		ds18b20_data_release();
		udelay(2);//释放总线至少1us，这里取2us
	}
	else
	{
		ds18b20_data_set_val_for_time(0, 2);//设置总线为低至少1us		
		ds18b20_data_release();
		udelay(60);
	}
}
...
static int ds18b20_read_bit(void)
{
	...
	ds18b20_data_set_val_for_time(0, 2);//设置总线为低至少1us		
	ds18b20_data_release();
	udelay(10);//15us之内来读数据
	val = ds18b20_data_get();//读取数据
	udelay(50);//一位至少为60us, 60 - 10 - 2 + 2 = 50 us
	return val;
}
...
int ds18b20_wait_when_processing(int timeout_us)
{
	while (timeout_us--)
	{
		if (ds18b20_read_bit() == 1)/* 如果转换成功则读到1 */
			return 0;  /* ok */
		udelay(1);
	}
	return -1;/*超时返回*/
...
int ds18b20_read_temperature(double *temp)
{
	...
	/* 先判断精度 */
	if (ram[4] & (3<<5) == 0) /* 0b00精度: 9bit */
		i = 3;
	else if (ram[4] & (3<<5) == (1<<5)) /* 0b01精度: 10bit */
		i = 2;
	else if (ram[4] & (3<<5) == (2<<5)) /* 0b10精度: 11bit */
		i = 1;
	else/* 0b11精度是 12 bit */
		i = 0;
