...
int do_master_tx(p_i2c_msg p_msg)
{
	...
	IICCON |= (1<<7);/*In Tx mode, the IICSDA is free in the ack time.*/
...
int do_master_rx(p_i2c_msg p_msg)
{
	p_cur_msg = p_msg;
	p_msg ->cnt_transferred = -1;
	p_msg->err = 0;
	...
	IICCON |= (1<<7);/*In Rx mode, the IICSDA is L in the ack time.*/
	/* 2. Write slave address to IICDS. */
