...
/* 3.3 分配显存(framebuffer),并把地址告诉LCD控制器 */
	...
	s3c_lcd->screen_base = dma_alloc_writecombine(NULL, s3c_lcd->fix.smem_len, &s3c_lcd->fix.mmio_start, GFP_KERNEL)
	
	lcd_regs->lcdsaddr1 = (s3c_lcd -> fix.smem_start >> 1) & (~(3<<30));//起始地址，只有29-0位有效//显存的物理地址
	lcd_regs->lcdsaddr1 = ((s3c_lcd -> fix.smem_star + s3c_lcd -> fix.smem_len)>>1)&(0x1ffff);//结束地址，只有21-0位有效
	lcd_regs->lcdsaddr3 = (240*16 / 16);//一行的长度( 单位是:半字，2字节 )
